...
for (const route of pages) {
  ...
  assert.match(
    html,
    /<div id="root">[\s\S]+<h1(?:\s|>)/,
    `${route}: page body must be prerendered`,
  );
  assert.equal(
    (html.match(/<h1(?:\s|>)/g) ?? []).length,
    1,
    `${route}: needs a single primary heading`,
  );
  assert.ok(
    html.includes(`<title>${escapeHtml(metadata.title)}</title>`),
    `${route}: expected page title`,
  );
  assert.ok(
    html.includes(
  ...
  assert.ok(
    html.includes(`<meta name="robots" content="${metadata.robots}"`),
    `${route}: expected indexing policy`,
  );
  assert.ok(
    html.includes(`<link rel="canonical" href="${metadata.canonical}"`),
    `${route}: expected canonical`,
  );
  assert.ok(
    html.includes(`<meta property="og:url" content="${metadata.canonical}"`),
    `${route}: expected sharing URL`,
  );
  assert.ok(
    html.includes('<meta name="twitter:card" content="summary"'),
    `${route}: expected Twitter card`,
  );
  for (const icon of [
    "/favicon.ico",
  ...
  assert.equal(
    structuredData["@graph"].find((item) => item["@type"] === "WebSite")?.url,
    `${SITE_URL}/`,
  );
  assert.ok(
    !html.includes("<!-- SEO_START -->"),
    `${route}: no unfinished template placeholders`,
  );
}
const heading = (route) =>
  pageHtml.get(route).match(/<h1(?:\s[^>]*)?>([\s\S]*?)<\/h1>/)?.[1];
assert.notEqual(
  heading("/"),
  heading("/404"),
  "404 must be a real error page, not a copy of the home page",
);
assert.notEqual(
  heading("/"),
  heading("/about"),
  "About must have its own page content",
);
assert.notEqual(
  heading("/"),
  heading("/explore"),
  "Explore must have its own page content",
);
assert.notEqual(
  heading("/404"),
  heading("/explore"),
  "Explore must render its mission overview, not the error page",
);
assert.notEqual(
  heading("/"),
  heading("/directory"),
  "Directory must have its own page content",
);
...
assert.deepEqual(
  ...
  Object.keys(routeMetadata)
    ...
    .map((route) => getPageMeta(route).canonical)
    .sort(),
...
for (const organization of organizations.filter(
  ...
  assert.ok(
    !sitemapUrls.some((url) => url.includes(`/business/${organization.id}/`)),
...
assert.ok(
  ...
  "Crawlers must be allowed to read page-level noindex directives",
);
const hostname = new URL(SITE_URL).hostname;
for (const file of ["CNAME", "public/CNAME", "dist/CNAME"]) {
  assert.equal(
    (await read(file)).trim(),
    hostname,
    `${file} must match SITE_URL`,
  );
}
for (const icon of [
  "favicon.ico",
